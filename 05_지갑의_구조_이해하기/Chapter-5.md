...
## Lesson 38. 트랜잭션 서명 권한을 분산하는 다중 서명 주소

보통 지갑 주소는 송금 등의 거래를 할 때 필요한 서명이 1개이다. 그런데 **송금할 때 서명이 여러 개 필요한 주소가 있다. 이를 다중 서명 주소(Multi Signature Address)라고 한다.** 비트코인은 지갑 주소의 시작 글자가 1이면 일반 지갑 주소, 3이면 다중 서명 주소이다.

모든 서명이 필요한 조건이라면, 하나의 키를 분실했을 경우 문제가 발생하게 된다. 이런 문제를 피하기 위해 일부의 서명이 모이면 된다는 `m of n 다중 서명` 이라는 운영 규칙을 두어 사용한다.

![image-20211226232351100](images/image-20211226232351100.png)